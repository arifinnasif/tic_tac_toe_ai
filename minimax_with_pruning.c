...
int minimax(int is_max, int alpha, int beta, int depth) {
char rep_char[] = {'o', ' ', '*'};
	for(int i = 0; i < N; i++) {
		for(int j = 0; j < N; j++) {
		...
		for(int i = 0; i < N; i++) {
			for(int j = 0; j < N; j++) {
				if(dup_board[i][j] == 0) {
					dup_board[i][j] = 1;
					dummy_count++;
					int t = minimax(0, alpha, beta, depth+1);
					if(t > val) {
						val = t;
						if(depth == 0) {
							move_row = i;
		...
		for(int i = 0; i < N; i++) {
			for(int j = 0; j < N; j++) {
				if(dup_board[i][j] == 0) {
					dup_board[i][j] = -1;
					dummy_count++;
					int t = minimax(1, alpha, beta, depth+1);
					if(t < val) {
						val = t;
						if(depth == 0) {
							move_row = i;
...
void ai_move(int **arg_board, int arg_is_max) {
	...
	minimax(arg_is_max, INT_MIN, INT_MAX, 0);
	delete_dup_board();
}
